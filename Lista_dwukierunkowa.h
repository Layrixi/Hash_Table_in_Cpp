...
public:
	string key;
	T* obj;//w  razie potrzeby się przeładuje operator gdyby trzeba było porówanć obiekty np
	list_obj<T>* next;
	list_obj<T>* prev;
	list_obj(string klucz)
	{
		key = klucz;
		obj = new T;
		next = NULL;
...
public:
	...
	list_obj<T>* find_by_key(string key);//USE ONLY IF YOUR OBJECT HAS A STRING KEY
	void print_list();
	linked_list()
	{
		head = NULL;
...
template <typename T> list_obj<T>* linked_list<T>::find_by_key(string key)
{
	list_obj<T>* checker = head;
	while (checker != NULL)
	{
		if (checker->obj->key == key)
			return checker;
		else
			checker = checker->next;
	}
...
template <typename T> bool linked_list<T>::remove_obj(T* obj)//usuwa element n-ty od początku
{
	list_obj<T>* obiekt = find_by_data(obj);
	if (obiekt == NULL)
		return false;
	if (obiekt == head)
	{
